# Git Shell Scripts

This repository contains useful shell scripts for Git operations and workflow automation.

- `git-switch-branch-orphan.sh`: Create a new orphan git branch

```bash
cd repo
git-switch-branch-orphan new-branch
```

- `git-checkout-branch-orphan.sh`: Alternative for old git version to create a new orphan git branch.

```bash
cd repo
git-checkout-branch-orphan new-branch
```
